...
def make_game():
    code = generate_code()
    rooms[code] = Game(code, 1) #tochange
    return code
...
def login(username, password):
    for login in logins:
        ...
        if password not in login:
            continue
        playersDict["aaaa"] = Player(username, 1, 15000)
        person = "aaaa"
...
def connect(data):
    ...
    session["room"] = lobbies

    # player = database.get_player(username, password)
    # if player is None:
    #     return "account doesn't exist"
    # else:
    #     session["player"] = Player(player[0]["username"], player[0]["playerID"], player[0]["money"])
    #     return "logged in!"
